...
class WindowClass(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        # [24.11.06 js] segmentation 추가
        self.actionSegmentation.triggered.connect(self.segmentation)
    ...
    def populateOID(self):
        ...
        # 객체 ID 리스트를 정렬하여 UI에 표시합니다.
        sorted_id_list = sorted(map(str, dict_Image[CurImg].idList))
        self.OIDmodel.setStringList(sorted_id_list)

        # QListView에 model 설정
        self.lv_OID.setModel(self.OIDmodel)

    # 클릭된 이미지의 json 정보를 가져옵니다.
    def filesItemClick(self, index):
        ...
        # 이미지에 해당하는 객체들의 리스트를 가져옵니다.
        # if dict_Image[CurImg].idList:
        #     last_oid = max(dict_Image[CurImg].idList)  # 가장 큰(마지막) ID를 선택
        #     CurObj = last_oid                          # 현 객체 ID를 선택된 ID로 설정
        # else:
        #     CurObj = None                               # 객체가 없는 경우 None으로 설정
        #     self.setKeypointsSpinBoxes(None)            # 키포인트 좌표 초기화(0으로 설정)

        CurObj = None
        self.setKeypointsSpinBoxes(None)
        
        if CurObj is not None:
            self.updateAnnotationView(CurObj)           # 선택된 객체의 정보를 업데이트
            self.OIDItemClick(CurObj)                   # 현 객체를 선택한 상태로 설정
            # 마지막 항목을 선택하고 배경색을 lightblue로 설정
            oid_index = self.OIDmodel.match(self.OIDmodel.index(0, 0), Qt.DisplayRole, str(CurObj), 1, Qt.MatchExactly)
            if oid_index:
                index = oid_index[0]
                self.lv_OID.selectionModel().setCurrentIndex(index, QItemSelectionModel.SelectCurrent)
    ...
    def setup_v_comboboxes(self):
        ...
        for name in self.keypoint_names:
            combobox = getattr(self, f"{name}_v_combobox")
            combobox.addItems(["1", "2"])
            combobox.currentIndexChanged.connect(self.on_v_value_changed)

    def setKeypointsSpinBoxes(self, OID: int):
        if OID is None:
            # 모든 키포인트 좌표와 v 값을 초기화
            self.lbl_pointCnt.setText("Keypoints : 0")
            print(self.lbl_crowdratio.text())
            for name in self.keypoint_names:
                getattr(self, f"{name}SpinBox").setValue(0)
                getattr(self, f"{name}SpinBox_2").setValue(0)
                getattr(self, f"{name}_v_combobox").setCurrentIndex(0)
        else:
            data = self.getJsonByOID(OID)
            if not data:
                print(f"Warning: No data found for OID {OID}")
                return
            
            data = data[0]
            self.lbl_pointCnt.setText(
                "Keypoints : " + str(len(data["keypoints"]) // 3)
            )
            for i, name in enumerate(self.keypoint_names):
                x, y, v = data["keypoints"][i*3:i*3+3]
                getattr(self, f"{name}SpinBox").setValue(x)
                getattr(self, f"{name}SpinBox_2").setValue(y)
                getattr(self, f"{name}_v_combobox").setCurrentIndex(v - 1 if v > 0 else 0)

    def on_v_value_changed(self):
        if CurObj is not None:
            self.updateKeypointsInJson()

    def updateKeypointsInJson(self):
        keypoints = []
        for name in self.keypoint_names:
            x = getattr(self, f"{name}SpinBox").value()
            y = getattr(self, f"{name}SpinBox_2").value()
            v = getattr(self, f"{name}_v_combobox").currentIndex() + 1
            keypoints.extend([x, y, v])
        
        self.mainJson.setKeypoints(CurObj, keypoints)
        self.updateAnnotationView(CurObj)
    ...
    def setKeypointsSpinBoxes(self, OID: int):
        ...
        if OID is None:
            ...
            self.lbl_crowdratio.setText('0.0')
            self.lbl_pointCnt.setText("Keypoints : 0")
            self.left_shoulderSpinBox.setValue(0)
            self.left_shoulderSpinBox_2.setValue(0)
            self.right_shoulderSpinBox.setValue(0)
            self.right_shoulderSpinBox_2.setValue(0)
            self.left_elbowSpinBox.setValue(0)
            self.left_elbowSpinBox_2.setValue(0)
            self.right_elbowSpinBox.setValue(0)
            self.right_elbowSpinBox_2.setValue(0)
            self.left_wristSpinBox.setValue(0)
            self.left_wristSpinBox_2.setValue(0)
            self.right_wristSpinBox.setValue(0)
            self.right_wristSpinBox_2.setValue(0)
            self.left_hipSpinBox.setValue(0)
            self.left_hipSpinBox_2.setValue(0)
            self.right_hipSpinBox.setValue(0)
            self.right_hipSpinBox_2.setValue(0)
            self.left_kneeSpinBox.setValue(0)
            self.left_kneeSpinBox_2.setValue(0)
            self.right_kneeSpinBox.setValue(0)
            self.right_kneeSpinBox_2.setValue(0)
            self.left_ankleSpinBox.setValue(0)
            self.left_ankleSpinBox_2.setValue(0)
            self.right_ankleSpinBox.setValue(0)
            self.right_ankleSpinBox_2.setValue(0)
            self.headSpinBox.setValue(0)
            self.headSpinBox_2.setValue(0)
            self.neckSpinBox.setValue(0)
            self.neckSpinBox_2.setValue(0)
            return
    ...
    def openDirectory(self):
        ...
        if dir:
            ...
            if file_paths:
                ...
                # self.populateList(file_path)
                self.loadImage(file_paths[0])
                # 처음 항목을 선택하고 배경색을 lightblue로 설정
                # iid_index는 선택된 이미지의 인덱스
                iid_index = self.filemodel.match(self.filemodel.index(0, 0), Qt.DisplayRole, str(CurImg), 1, Qt.MatchExactly)
                if iid_index:
                    # 선택된 이미지의 인덱스를 가져옵니다.
                    index = iid_index[0]
                    # 선택된 이미지의 인덱스를 선택한 상태로 설정해서 배경색을 lightblue로 설정합니다.
                    self.lv_files.selectionModel().setCurrentIndex(index, QItemSelectionModel.SelectCurrent)
    ...
    def convert_json_file(self):
        ...
        if action == posetrack_action:
            ...
            if input_file:
                image_folder = QFileDialog.getExistingDirectory(self, "Select Image Directory")
                if image_folder:
                    # 저장할 파일 경로 선택
                    output_file, _ = QFileDialog.getSaveFileName(self, "Save Converted JSON File", "", "JSON Files (*.json)")
                    if output_file:
                        self.process_posetrack_json(input_file, image_folder, output_file)
    ...
    def is_within_ignore_regions(self, x, y, ignore_regions_x, ignore_regions_y):
        for poly_x, poly_y in zip(ignore_regions_x, ignore_regions_y):
    ...
    def convert_posetrack_keypoints(self, posetrack_keypoints):
        mapping = {5: 0, 6: 1, 7: 2, 8: 3, 9: 4, 10: 5, 11: 6, 12: 7, 13: 8, 14: 9, 15: 10, 16: 11}
        crowdpose_keypoints = [0] * (14 * 3)

        for src, dst in mapping.items():
            src_idx = src * 3
            dst_idx = dst * 3
            crowdpose_keypoints[dst_idx:dst_idx+3] = posetrack_keypoints[src_idx:src_idx+3]

        # Head
        head_top = posetrack_keypoints[2*3:2*3+3]
        head_bottom = posetrack_keypoints[1*3:1*3+3]

        if head_top[2] > 0 and head_bottom[2] > 0:
            # 1) head_top, head_bottom이 모두 존재할 경우 중간값을 head로 지정
            head_x = (head_top[0] + head_bottom[0]) / 2
            head_y = (head_top[1] + head_bottom[1]) / 2
            crowdpose_keypoints[12*3:12*3+3] = [head_x, head_y, 1]
        elif head_top[2] > 0:
            # 2) head_top만 존재하는 경우 해당 값을 head로 지정
            crowdpose_keypoints[12*3:12*3+3] = head_top
        elif head_bottom[2] > 0:
            # 3) head_bottom만 존재하는 경우 해당 값을 head로 지정
            crowdpose_keypoints[12*3:12*3+3] = head_bottom
        # 4) 둘 다 존재하지 않는 경우 0으로 지정

        # Neck
        left_shoulder = posetrack_keypoints[5*3:5*3+3]
        right_shoulder = posetrack_keypoints[6*3:6*3+3]

        if head_bottom[2] > 0:
            # head_bottom이 존재하고
            neck_x = head_bottom[0]
            if left_shoulder[2] > 0 and right_shoulder[2] > 0:
                # 1) 두 어깨가 모두 존재할 경우 head_bottom와 두어깨의 중간값의 중간값을 neck으로 지정
                shoulder_y = (left_shoulder[1] + right_shoulder[1]) / 2
                neck_y = (head_bottom[1] + shoulder_y) / 2
            elif left_shoulder[2] > 0:
                # 2) 한쪽 어깨만 존재할 경우 해당 어깨의 y좌표와 head_bottom의 중간값을 y좌표로 지정
                neck_y = (head_bottom[1] + left_shoulder[1]) / 2
            elif right_shoulder[2] > 0:
                neck_y = (head_bottom[1] + right_shoulder[1]) / 2
            else:
                # 3) 두 어깨 모두 존재하지 않는 경우 head_bottom으로 지정
                neck_y = head_bottom[1]
            crowdpose_keypoints[13*3:13*3+3] = [neck_x, neck_y, 1]
        elif left_shoulder[2] > 0 and right_shoulder[2] > 0:
            # 4) head_bottom이 존재하지 않고 어깨만 존재하는 경우 두 어깨의 중간값을 neck으로 지정
            neck_x = (left_shoulder[0] + right_shoulder[0]) / 2
            neck_y = (left_shoulder[1] + right_shoulder[1]) / 2
            crowdpose_keypoints[13*3:13*3+3] = [neck_x, neck_y, 1]
        # 4) 둘 다 존재하지 않는 경우 0으로 지정

        return crowdpose_keypoints

    def process_posetrack_json(self, json_file, image_folder, output_file):
        try:
            with open(json_file, 'r') as f:
                data = json.load(f)

            # Set width and height from the first image in the directory
            first_image_path = glob.glob(os.path.join(image_folder, "*"))[0]
            ...
            for image in data['images']:
                ...
                # New image_id is file name without extension, as an integer without quotes
                new_image_id = int(os.path.splitext(file_name)[0])
            ...
            for anno in data.get('annotations', []):
                ...
                image = next((img for img in data['images'] if img['id'] == original_image_id), None)
                if image is None:
                    continue

                new_image_id = int(os.path.splitext(os.path.basename(image['file_name']))[0])
                keypoints = anno.get('keypoints', [])

                ignore_regions_x = image.get('ignore_regions_x', [])
                ignore_regions_y = image.get('ignore_regions_y', [])

                for i in range(0, len(keypoints), 3):
                    x, y, v = keypoints[i:i+3]
                    if v > 0 and self.is_within_ignore_regions(x, y, ignore_regions_x, ignore_regions_y):
                        keypoints[i] = keypoints[i+1] = keypoints[i+2] = 0

                keypoints = [int(value) if i % 3 != 2 else value for i, value in enumerate(keypoints)]

                crowdpose_keypoints = self.convert_posetrack_keypoints(keypoints)

                bbox = anno.get('bbox', [0, 0, width, height])
            ...
            for image in crowdpose_data["images"]:
                image_id = image['id']
                objs = [i for i in crowdpose_data["annotations"] if i["image_id"] == image_id]
        ...
        try:
            with open(input_file, 'r') as file:
                ...
                for coco_annotation in coco_data["annotations"]:
                    ...
                    left_shoulder = keypoints[3*5:3*5+3]
                    right_shoulder = keypoints[3*6:3*6+3]
                    nose = keypoints[0:3]
                    left_eye = keypoints[3*1:3*1+3]
                    right_eye = keypoints[3*2:3*2+3]

                    # Neck 키포인트 생성

                    # 1) 양 쪽 어깨 키포인트가 모두 존재할 경우 두 키포인트의 중간값으로 지정
                    if left_shoulder[2] > 0 and right_shoulder[2] > 0:
                        neck_x = (left_shoulder[0] + right_shoulder[0]) / 2
                        neck_y = (left_shoulder[1] + right_shoulder[1]) / 2
                        neck_score = (left_shoulder[2] + right_shoulder[2]) / 2

                    # 2) 한 쪽 어깨 키포인트만 존재할 경우 
                    elif left_shoulder[2] > 0 or right_shoulder[2] > 0:
                        shoulder = left_shoulder if left_shoulder[2] > 0 else right_shoulder
                        neck_y = shoulder[1]

                        # 2-1) 코 키포인트가 존재하면 코 키포인트의 x 좌표, 한 쪽 어깨의 y좌표
                        if nose[2] > 0:
                            neck_x = nose[0]
                        
                        # 2-2) 코 키포인트가 없을 때, 두 눈의 키포인트가 존재하면 
                        # 두 눈의 중간값의 x 좌표, 한 쪽 어깨의 y좌표
                        elif left_eye[2] > 0 and right_eye[2] > 0:
                            neck_x = (left_eye[0] + right_eye[0]) / 2
                        neck_score = shoulder[2]

                    # 3) 어깨 키포인트가 없는 경우 0으로 지정   
                    else:
                        neck_x = neck_y = neck_score = 0
                    ...
                    # 1) 코 키포인트가 존재할 경우 코의 키포인트로 지정
                    if nose[2] > 0:
                        head_x, head_y, head_score = nose
                    
                    # 2) 코 키포인트가 없을 때, 두 눈의 키포인트가 존재할 경우
                    #    두 눈의 중간값으로 지정
                    elif left_eye[2] > 0 and right_eye[2] > 0:
                        head_x = (left_eye[0] + right_eye[0]) / 2
                        head_y = (left_eye[1] + right_eye[1]) / 2
                        head_score = (left_eye[2] + right_eye[2]) / 2
                    
                    # 3) 한 쪽 눈의 키포인트만 존재할 경우 해당 키포인트로 지정
                    elif left_eye[2] > 0:
                        head_x, head_y, head_score = left_eye
                    elif right_eye[2] > 0:
                        head_x, head_y, head_score = right_eye
                    
                    # 4) 눈, 코의 키포인트가 존재하지 않는 경우 0으로 지정
                    else:
                        head_x = head_y = head_score = 0

                    crowdpose_keypoints[12 * 3] = head_x
                    crowdpose_keypoints[12 * 3 + 1] = head_y
                    crowdpose_keypoints[12 * 3 + 2] = head_score
    ...
    # [24.11.06 js] Segmentation 정보를 추가하는 함수
    def segmentation(self):
        new_file_path, _ = QFileDialog.getSaveFileName(self, "Save", "", "JSON Files (*.json)")
        if new_file_path:
            try:
                updated_json = self.mainJson.segmentation()
                with open(new_file_path, 'w') as file:
                    json.dump(updated_json, file, indent=4)

                QMessageBox.information(self, "Update Result", "Segmentation Completed")
            except Exception as e:
                QMessageBox.critical(self, "Save Error", f"Save Error:\n{str(e)}")
