from PyQt5.QtWidgets import QGraphicsRectItem, QGraphicsView, QGraphicsScene, QGraphicsEllipseItem, QGraphicsItem, QStyle, QMenu
from PyQt5.QtCore import QRectF, Qt, QStringListModel, QItemSelectionModel
...
class CustomListModel(QStringListModel):
    ...
    def data(self, index, role=Qt.DisplayRole, color=QBrush(QColor('white'))):
        if role == Qt.DisplayRole:
            # index에 해당하는 텍스트를 반환합니다.
            # return self.items[index.row()]
            pass
        elif role == Qt.BackgroundRole:
            # 저장된 배경색이 있으면 반환하고, 없으면 기본 배경색을 반환합니다.
            return self.itemBackgroundColors.get(index.row(), color)
        return super().data(index, role)

    def setData(self, index, value, role=Qt.EditRole):
        if index != 0 and role == Qt.BackgroundRole:
            self.itemBackgroundColors[index.row()] = value  # 배경색을 저장합니다.
            self.dataChanged.emit(index, index, [role])
            return True
        return super().setData(index, value, role)
