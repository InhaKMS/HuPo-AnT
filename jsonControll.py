...
class CrowdPoseJson:
    ...
    def update_duplicate_ids(self):
        ...
        # 중복된 id를 가진 객체들의 id를 새로운 id로 변경
        for annotation in self.after_json['annotations']:
            ann_id = annotation['id']
            if id_count[ann_id] > 1:
                duplicated_id.add(ann_id)
                duplicated_image_id.add(annotation['image_id'])
                max_id += 1
        ...
        return updated_json, duplicated_id, duplicated_image_id

    # def FitJSON(self):
    #     import main
    #     ids = [i["id"] for i in main.dict_Image.values()]
    #     print(main.dict_Image)
    #     self.after_json["annotations"] = [i for i in self.after_json["annotations"] if i["image_id"] in ids]
